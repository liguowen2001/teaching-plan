...
public class UserSpecs {
    ...
    public static Specification<User> containingRole(Long role) {
        if (role != null) {
            return (Specification<User>) (root, criteriaQuery, criteriaBuilder) -> criteriaBuilder.like(root.get("role").as(String.class), String.format("%%%s%%", role));
        } else {
            return Specification.where(null);
        }
    }
